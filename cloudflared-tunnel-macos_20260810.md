...
## 12. codebuddy Web UI 无输入框（launchd 不传 QClaw 鉴权变量）

**现象**：手机经隧道打开 `codebuddy.54715471.xyz`，页面能进但**没有输入框、是只读状态**；昨天在 QClaw 终端里手动起的同服务却有输入框。

**根因**（已验证）：
- codebuddy 的账号身份 + LLM key + 网关地址，依赖一批由 **QClaw 运行时注入到它拉起进程 environment** 里的环境变量（`QCLAW_*`、`AUTH_GATEWAY_PORT`，共 35 个）。
- 这些变量**不在** `launchctl setenv`（实测 `launchctl getenv QCLAW_*` 为空），也不在任何 rc 文件里——是 QClaw 直接 `exec` 时塞进子进程环境的。
- launchd 起的 agent 进程环境是空的（只有 `HOME` + 极简 `PATH`），拿不到这批变量 → codebuddy 以**匿名/只读**模式启动 → Web UI 无输入框（本地直连返 `403`，登录态正常应是 `200`）。
- 昨天手动起是在 QClaw 终端里，继承了父进程 env，所以正常。

**修法**（已实施并验证）：用 wrapper 脚本在启动时把变量灌进去，再把 plist 指向 wrapper。

1. 落盘 env 文件（从能拿到变量的环境）：
   ```bash
   env | grep -E '^(QCLAW_|AUTH_GATEWAY_)' | while IFS='=' read -r k v; do
     printf "%s='%s'\n" "$k" "$v"   # 值必须单引号包裹！路径含空格(Application Support)
   done > ~/.local/share/codebuddy-env.sh
   chmod 600 ~/.local/share/codebuddy-env.sh
   ```
2. wrapper `/Users/momo/.local/bin/codebuddy-serve.sh`：
   ```zsh
   #!/bin/zsh
   ENV_FILE="/Users/momo/.local/share/codebuddy-env.sh"
   [ -f "$ENV_FILE" ] && { set -a; source "$ENV_FILE"; set +a; }
   export HOME="/Users/momo"
   export PATH="/usr/local/bin:/usr/bin:/bin:/usr/sbin:/sbin"
   exec /usr/local/bin/node /usr/local/lib/node_modules/@tencent-ai/codebuddy-code/bin/codebuddy \
     --serve --host 127.0.0.1 --port 50000
   ```
3. plist 改调 wrapper：
   ```xml
   <key>ProgramArguments</key>
   <array>
     ...
     <string>/Users/momo/.local/bin/codebuddy-serve.sh</string>
   </array>
   ```
4. 重载：`launchctl unload ...plist && launchctl load ...plist`

**验证**：
- wrapper 内 `exec` 前计数 `env | grep -cE '^(QCLAW_|AUTH_GATEWAY_)'` = **35**，`QCLAW_USER_TOKEN_ENCRYPTED` 长度 448（匿名时为 0）。
- 本地 `curl 127.0.0.1:50000` 从 `403` → **`200`**（只读→已登录态）。
- 手机重开隧道地址应出现输入框。

**踩坑**：
- 直接 `env | grep ... > file` 然后 `source file` 会报 `no such file or directory: Support/QClaw/...`——因为值里的空格把命令拆了。**必须单引号包裹值**。
- `ps eww -p <pid>` 在这种 env 下解析不可靠（空格把变量名切碎，显示 0 个），别被它骗了；以 wrapper 内计数和 `curl` 状态码为准。
- 别指望 `launchctl getenv` 拿 QClaw 变量——它在 launchd 域里是空的，必须落盘 env 文件。

**注意**：env 文件含登录 token，权限 `600`；QClaw 登出/换号后 token 会变，需重新生成此文件（wrapper 启动时会 source 最新版）。
